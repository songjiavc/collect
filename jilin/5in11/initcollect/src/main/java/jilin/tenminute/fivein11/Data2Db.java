...
public class Data2Db {
	...
	public List<SrcDataBean> getAllRecords(){
		...
		String sql = "SELECT issue_id,no_1,no_2,no_3,no_4,no_5 FROM echart_jilin_11xuan5_t";
		try {
			pstmt = (PreparedStatement) srcConn.prepareStatement(sql);
			ResultSet rs = pstmt.executeQuery();
			while(rs.next()){
	...
	public SrcDataBean getRecordByIssueId(String issueId){
		...
		PreparedStatement pstmt = null;
		String sql = "SELECT issue_id,no_1,no_2,no_3,no_4,no_5 FROM echart_jilin_11xuan5_t WHERE issue_id = '"+issueId+"'";
		try {
			pstmt = (PreparedStatement) srcConn.prepareStatement(sql);
			ResultSet rs = pstmt.executeQuery();
			while(rs.next()){
