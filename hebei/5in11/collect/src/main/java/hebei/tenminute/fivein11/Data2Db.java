...
public class Data2Db {
	...
	public String findMaxIssueIdFromDesDb(){
		...
		String issueId = null;
		PreparedStatement pstmt = null;
		String sql = "SELECT max(ISSUE_NUMBER) FROM T_HEBEI_5IN11_NUMBER";
		try {
	...
	public List<SrcDataBean> getAllRecords(){
		...
		String sql = "SELECT issue_id,no_1,no_2,no_3,no_4,no_5 FROM echart_hebei_11xuan5_t where issue_id > '16010101'";
		try {
			pstmt = (PreparedStatement) srcConn.prepareStatement(sql);
			ResultSet rs = pstmt.executeQuery();
			while(rs.next()){
	...
	public SrcDataBean getRecordByIssueId(String issueId){
		...
		PreparedStatement pstmt = null;
		String sql = "SELECT issue_id,no_1,no_2,no_3,no_4,no_5  FROM echart3.echart_hebei_11xuan5_t WHERE issue_id = '"+issueId+"'";
		try {
			pstmt = (PreparedStatement) srcConn.prepareStatement(sql);
			ResultSet rs = pstmt.executeQuery();
			while(rs.next()){
...
//				LogUtil.info(srcDataBean.getIssueId()+"插入成功!");
	...
	private void insertData(SrcDataBean srcDataBean,Connection conn) throws SQLException{
		String sql = "INSERT INTO T_HEBEI_5IN11_NUMBER (issue_number,no1,no2,no3,no4,no5,three_sum,three_span,five_sum,five_span,big_count,odd_count,create_time,origin) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		PreparedStatement pstmt = null;
		try {
	...
	private boolean haveDataInIssueId(String issueId,Connection conn) throws SQLException{
		boolean flag = false;
		int count = 0;
		String sql = "SELECT COUNT(*) FROM T_HEBEI_5IN11_NUMBER WHERE ISSUE_NUMBER = '"+issueId+"'";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			pstmt = (PreparedStatement) conn.prepareStatement(sql);
