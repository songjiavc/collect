...
public class AnalysisMissUtil {
	...
	public static List<String> updateGroupMiss(SrcDataBean srcDataBean, int n) {
		...
		sqlList.add("UPDATE T_HEBEI_5IN11_MISSANALYSIS SET CURRENT_MISS = 0 WHERE TYPE = " + n + "  AND GROUP_NUMBER IN ("
				+ inStr + ")");
		if(n == 3){
			//任三四码复式
			...
			for(String likeGroup : likeGroupList){
				//任三四码
				sqlList.add("UPDATE T_HEBEI_5IN11_MISSANALYSIS SET OPTIONAL_COMPOUND = 0 WHERE TYPE = 4 AND GROUP_NUMBER LIKE ("+ likeGroup + ")");
			}
		}else if(n == 4){
			List<String > likeGroupList = getLikeGroupByNumber(noArr,n);
			for(String likeGroup : likeGroupList){
				//任四五码复式
				sqlList.add("UPDATE T_HEBEI_5IN11_MISSANALYSIS SET OPTIONAL_COMPOUND = 0 WHERE TYPE IN (5,6) AND GROUP_NUMBER LIKE ("+ likeGroup + ")");
			}
		}
		return sqlList;
	}
	...
	public static String updateGreatFiveGroupMiss(SrcDataBean srcDataBean, int n) {
		String rtnSql = null;
		int[] noArr = getIntArr(srcDataBean);
		rtnSql = "UPDATE T_HEBEI_5IN11_MISSANALYSIS SET CURRENT_MISS = 0 WHERE TYPE = " + n + "  AND GROUP_NUMBER LIKE  '%"
	...
	public static String[] updateBeforeRen2GroupMiss(SrcDataBean srcDataBean) {
		...
		rtnSql[0] = "UPDATE T_HEBEI_5IN11_MISSANALYSIS SET CURRENT_MISS = 0 WHERE TYPE = 9  AND GROUP_NUMBER = '"+ translate(noArr[0]) + translate(noArr[1]) + "'";
		rtnSql[1] = "UPDATE T_HEBEI_5IN11_MISSANALYSIS SET TWOCODE_COMPOUND = 0 WHERE TYPE IN (3,4,5,6,7)  AND GROUP_NUMBER LIKE '%" + translate(noArr[0]) + "%" + translate(noArr[1]) +  "%'";
		return rtnSql;
	}
	...
	public static String[] updateBeforeRen3GroupMiss(SrcDataBean srcDataBean) {
		...
		rtnSql[0] = "UPDATE T_HEBEI_5IN11_MISSANALYSIS SET CURRENT_MISS = 0 WHERE TYPE = 10  AND GROUP_NUMBER = '"
				+ translate(noArr[0]) + translate(noArr[1]) + translate(noArr[2]) + "'";
		rtnSql[1] = "UPDATE T_HEBEI_5IN11_MISSANALYSIS SET THREECODE_COMPOUND = 0 WHERE TYPE IN (4,5,6,7,8)  AND GROUP_NUMBER LIKE '%" + translate(noArr[0]) + "%"  + translate(noArr[1]) + "%"+ translate(noArr[2]) +  "%'";
		return rtnSql;
	}

	public static String updateDirectBeforeRen2GroupMiss(SrcDataBean srcDataBean) {
		String rtnSql = null;
		rtnSql = "UPDATE T_HEBEI_5IN11_MISSANALYSIS SET CURRENT_MISS = 0 WHERE TYPE = 11  AND GROUP_NUMBER = '"
	...
	public static String updateDirectBeforeRen3GroupMiss(SrcDataBean srcDataBean) {
		String rtnSql = null;
		rtnSql = "UPDATE T_HEBEI_5IN11_MISSANALYSIS SET CURRENT_MISS = 0 WHERE TYPE = 12  AND GROUP_NUMBER = '"
