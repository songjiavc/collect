...
public class Data2Db {
	...
	public String findMaxIssueIdFromSrcDb(){
		...
		String sql = "SELECT max(issue_number) FROM T_HLJ_5IN11_NUMBER";
		try {
	...
	/** 
	  ...
	  * @return 
	  */
	public List<SrcDataBean> getAllNumber(){
		Connection srcConn = ConnectSrcDb.getSrcConnection();
		
		List<SrcDataBean> dataList = new ArrayList<SrcDataBean>();
		PreparedStatement pstmt = null;
		String sql = "SELECT issue_number,no1,no2,no3,no4,no5  FROM T_HLJ_5IN11_NUMBER ORDER BY ISSUE_NUMBER ASC";
		try {
			pstmt = (PreparedStatement) srcConn.prepareStatement(sql);
			ResultSet rs = pstmt.executeQuery();
			while(rs.next()){
	...
	public SrcDataBean getRecordByIssueId(String issueId){
		...
		SrcDataBean srcDataBean = new SrcDataBean();
		PreparedStatement pstmt = null;
		String sql = "SELECT issue_number,no1,no2,no3,no4,no5  FROM T_HLJ_5IN11_NUMBER WHERE issue_number = '"+issueId+"'";
		try {
			pstmt = (PreparedStatement) srcConn.prepareStatement(sql);
			ResultSet rs = pstmt.executeQuery();
			while(rs.next()){
	...
	public void insertTestGroupData(String issueNumber,String groupNumber,int currentMiss,int maxMiss,int type) throws SQLException{
		Connection conn = ConnectDesDb.getDesConnection();
		String sql = "insert into T_HLJ_5IN11_MISSANALYSIS_TEST (ISSUE_NUMBER,GROUP_NUMBER,CURRENT_MISS,MAX_MISS,TYPE) values(?,?,?,?,?)";
		PreparedStatement pstmt = null;
		try {
	...
	private boolean haveMissDataInIssueId(String issueId,Connection conn) throws SQLException {
		boolean flag = false;
		int count = 0;
		String sql = "SELECT COUNT(*) FROM T_HLJ_5IN11_MISSANALYSIS_TEST WHERE ISSUE_NUMBER = '"+issueId+"'";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			pstmt = (PreparedStatement) conn.prepareStatement(sql);
	...
	private void batchUpdateMiss(SrcDataBean srcDataBean,Connection conn) throws SQLException{
		...
		try{
			...
			if(a){
				  ...
				  stmt.addBatch("UPDATE T_HLJ_5IN11_MISSANALYSIS_TEST SET ISSUE_NUMBER = "+srcDataBean.getIssueId()+","
				  ...
				  //同时提交所有的sql语句
				  stmt.addBatch("UPDATE T_HLJ_5IN11_MISSANALYSIS_TEST SET MAX_MISS = CURRENT_MISS WHERE CURRENT_MISS > MAX_MISS AND CURRENT_MISS <> 0;");
				  stmt.addBatch("UPDATE T_HLJ_5IN11_MISSANALYSIS_TEST SET TWOCODE_COMPOUND_MAXMISS = TWOCODE_COMPOUND WHERE TWOCODE_COMPOUND > TWOCODE_COMPOUND_MAXMISS AND TWOCODE_COMPOUND <> 0;");
				  stmt.addBatch("UPDATE T_HLJ_5IN11_MISSANALYSIS_TEST SET THREECODE_COMPOUND_MAXMISS = THREECODE_COMPOUND WHERE THREECODE_COMPOUND > THREECODE_COMPOUND_MAXMISS AND THREECODE_COMPOUND <> 0;");
				  stmt.addBatch("UPDATE T_HLJ_5IN11_MISSANALYSIS_TEST SET OPTIONAL_COMPOUND_MAXMISS = OPTIONAL_COMPOUND WHERE OPTIONAL_COMPOUND > OPTIONAL_COMPOUND_MAXMISS AND OPTIONAL_COMPOUND <> 0;");
				  stmt.executeBatch();
				  //提交修改
				  conn.commit();
