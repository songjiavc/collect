...
public class Data2Db {
	...
	public String findMaxIssueIdFromSrcDb(){
		...
		String sql = "SELECT max(ISSUE_NUMBER) FROM T_LN_5IN12_NUMBER";
		try {
	...
	public SrcDataBean getRecordByIssueId(String issueId){
		...
		PreparedStatement pstmt = null;
		String sql = "SELECT ISSUE_NUMBER,NO1,NO2,NO3,NO4,NO5  FROM T_LN_5IN12_NUMBER WHERE ISSUE_NUMBER = '"+issueId+"'";
		try {
			pstmt = (PreparedStatement) srcConn.prepareStatement(sql);
			ResultSet rs = pstmt.executeQuery();
			while(rs.next()){
		...
		//由于不需要插入基础数据内容，所以直接取消掉数据以外其他统计的内容
...
//		srcDataBean.setFiveSum(fiveSum);
	...
	private boolean haveMissDataInIssueId(String issueId,Connection conn) throws SQLException {
		boolean flag = false;
		int count = 0;
		String sql = "SELECT COUNT(*) FROM T_LN_5IN12_MISSANALYSIS WHERE ISSUE_NUMBER = '"+issueId+"'";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			pstmt = (PreparedStatement) conn.prepareStatement(sql);
...
//					  System.out.println(sql);
				  ...
				  //同时提交所有的sql语句
				  stmt.addBatch("UPDATE T_LN_5IN12_MISSANALYSIS SET MAX_MISS = CURRENT_MISS WHERE CURRENT_MISS > MAX_MISS AND CURRENT_MISS <> 0;");
				  stmt.addBatch("UPDATE T_LN_5IN12_MISSANALYSIS SET TWOCODE_COMPOUND_MAXMISS = TWOCODE_COMPOUND WHERE TWOCODE_COMPOUND > TWOCODE_COMPOUND_MAXMISS AND TWOCODE_COMPOUND <> 0;");
				  stmt.addBatch("UPDATE T_LN_5IN12_MISSANALYSIS SET THREECODE_COMPOUND_MAXMISS = THREECODE_COMPOUND WHERE THREECODE_COMPOUND > THREECODE_COMPOUND_MAXMISS AND THREECODE_COMPOUND <> 0;");
				  stmt.addBatch("UPDATE T_LN_5IN12_MISSANALYSIS SET OPTIONAL_COMPOUND_MAXMISS = OPTIONAL_COMPOUND WHERE OPTIONAL_COMPOUND > OPTIONAL_COMPOUND_MAXMISS AND OPTIONAL_COMPOUND <> 0;");
				  stmt.executeBatch();
				  //提交修改
				  conn.commit();
