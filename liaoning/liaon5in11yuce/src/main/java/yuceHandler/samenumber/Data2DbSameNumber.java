...
//			Integer[] numIntArr = getUniqueArr(temp);           //不需要去重处理
    ...
    private void insertData2Db(List<String> issueList,List<SrcDataBean> noList,List<Fast3SameNumber> fast3CountList) throws SQLException{
		...
		for(int i = noList.size()-1;i>=0;i--){
			if(i == 0){
				pstmt.setString(4,App.translate(fast3CountList.get(0).getNumber())+App.translate(fast3CountList.get(1).getNumber())+App.translate(fast3CountList.get(9).getNumber())+App.translate(fast3CountList.get(10).getNumber()));
			}else{
