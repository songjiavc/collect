...
//			Integer[] numIntArr = getUniqueArr(temp);           //不需要去重处理
    ...
    private void insertData2Db(List<String> issueList,List<SrcDataBean> noList,List<Fast3SameNumber> fast3CountList) throws SQLException{
		...
		for(int i = noList.size()-1;i>=0;i--){
			if(i == 0){
				pstmt.setString(4,App.translate(fast3CountList.get(0).getNumber())+App.translate(fast3CountList.get(1).getNumber())+App.translate(fast3CountList.get(10).getNumber())+App.translate(fast3CountList.get(11).getNumber()));
			}else{
				pstmt.setString(4, App.translate(noList.get(i).getNo1())+App.translate(noList.get(i).getNo2())+App.translate(noList.get(i).getNo3())+App.translate(noList.get(i).getNo4())+App.translate(noList.get(i).getNo5()));
			}
