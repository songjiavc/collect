...
public class Data2Db {
	...
	public String findMaxIssueIdFromSrcDb(){
		...
		String sql = "SELECT max(issue_id) FROM echart3.echart_liaoning_11xuan5_t";
		try {
	...
	public SrcDataBean getRecordByIssueId(String issueId){
		...
		SrcDataBean srcDataBean = new SrcDataBean();
		PreparedStatement pstmt = null;
		String sql = "SELECT issue_id,no_1,no_2,no_3,no_4,no_5  FROM echart3.echart_liaoning_11xuan5_t WHERE issue_id = '"+issueId+"'";
		try {
			pstmt = (PreparedStatement) srcConn.prepareStatement(sql);
			ResultSet rs = pstmt.executeQuery();
			while(rs.next()){
	...
	private void insertData(SrcDataBean srcDataBean,Connection conn) throws SQLException{
		String sql = "INSERT INTO T_LN_5IN11_NUMBER (issue_number,no1,no2,no3,no4,no5,three_sum,three_span,five_sum,five_span,big_count,odd_count,create_time,origin) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		PreparedStatement pstmt = null;
		try {
	...
	public void insertTestGroupData(String issueNumber,String groupNumber,int currentMiss,int maxMiss,int type) throws SQLException{
		Connection conn = ConnectDesDb.getDesConnection();
		String sql = "insert into T_LN_5IN11_MISSANALYSIS (ISSUE_NUMBER,GROUP_NUMBER,CURRENT_MISS,MAX_MISS,TYPE) values(?,?,?,?,?)";
		PreparedStatement pstmt = null;
		try {
			pstmt = (PreparedStatement) conn.prepareStatement(sql);
	...
	private boolean haveMissDataInIssueId(String issueId,Connection conn) throws SQLException {
		boolean flag = false;
		int count = 0;
		String sql = "SELECT COUNT(*) FROM T_LN_5IN11_MISSANALYSIS WHERE ISSUE_NUMBER = '"+issueId+"'";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			pstmt = (PreparedStatement) conn.prepareStatement(sql);
	...
	private void batchUpdateMiss(SrcDataBean srcDataBean,Connection conn) throws SQLException{
		...
		try{
			...
			if(a){
				  ...
				  stmt.addBatch("UPDATE T_LN_5IN11_MISSANALYSIS SET ISSUE_NUMBER = "+srcDataBean.getIssueId()+","
				  ...
				  //同时提交所有的sql语句
				  stmt.addBatch("UPDATE T_LN_5IN11_MISSANALYSIS SET MAX_MISS = CURRENT_MISS WHERE CURRENT_MISS > MAX_MISS AND CURRENT_MISS <> 0;");
				  stmt.addBatch("UPDATE T_LN_5IN11_MISSANALYSIS SET TWOCODE_COMPOUND_MAXMISS = TWOCODE_COMPOUND WHERE TWOCODE_COMPOUND > TWOCODE_COMPOUND_MAXMISS AND TWOCODE_COMPOUND <> 0;");
				  stmt.addBatch("UPDATE T_LN_5IN11_MISSANALYSIS SET THREECODE_COMPOUND_MAXMISS = THREECODE_COMPOUND WHERE THREECODE_COMPOUND > THREECODE_COMPOUND_MAXMISS AND THREECODE_COMPOUND <> 0;");
				  stmt.addBatch("UPDATE T_LN_5IN11_MISSANALYSIS SET OPTIONAL_COMPOUND_MAXMISS = OPTIONAL_COMPOUND WHERE OPTIONAL_COMPOUND > OPTIONAL_COMPOUND_MAXMISS AND OPTIONAL_COMPOUND <> 0;");
				  stmt.executeBatch();
				  //提交修改
				  conn.commit();
